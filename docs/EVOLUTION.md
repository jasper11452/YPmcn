...
4. **收紧外部副作用的证据**：当前 `create_with_distributions` 每次都会先经过插件的本地确认，确认回执只放行一次实际调用；“是否已发送”仍只能依赖逐机构的 Provider 回执，不能依赖弹窗、Local JSON 或后续 sync。
...
一个典型变化：外发确认和外发成功是两件事。当前弹窗说明媒介确认了发送意图，随后的一次调用才会被放行；它仍不能证明任何消息已经到达。今天要判断外发成功，必须看 Provider 返回的、可对应每个供应商的 `sent` 明细。
...
- 索引中某些旧标题含“直接续发”等措辞时，它们描述的是当时的变更主题；当前外发行为以 `spec/workflow.json` 的确认门禁为准。
