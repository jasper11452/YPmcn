...
`sync_mcn_inquiry_status(requirement_id, project_id, mcn_id)` 的最小实现：

1. 用成功的 create ledger 证明 project 属于当前 requirement；
2. 读取 `core_project` 和唯一 `(project_id,supplier_id)` 对应的 `core_distribution`；
3. 用 `mcn_recommendation_item_id + attempt_no` 现有唯一键 upsert `mcn_inquiries`，写发送时间、截止、状态、提交时间和行数；
4. 用 `(requirement_id,project_id,mcn_id)` 现有唯一键更新 `mcn_inquiry_status_syncs` 的最后同步时间；
5. 相同 distribution 状态重复同步返回同一个 `inquiry_id`，不新增记录。
