...
1. `npm run test:fast`：源码 Plugin/Hook/契约回归，验证非外发 Tool 放行、本地 JSON 状态转换与企微外发 Native Approval。
2. `npm run verify:provider`：直接对开发 SSE 做 `initialize + tools/list` 契约快照，发现工具/参数漂移。
3. Hook 响应回放：用脱敏外发参数验证 Native Approval 绑定原 Tool 调用、请求指纹、参数变化及 unknown 后重新审批，不需要启动桌面端。
