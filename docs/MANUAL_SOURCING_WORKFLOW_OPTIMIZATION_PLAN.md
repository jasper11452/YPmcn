...
> 2026-07-21 决策修正（CHG-2026-016）：本文后续 3.4.0 细节仅保留为历史设计记录，不再作为当前手扒数量决策依据。当前规则是：赛马前只展示刊例资源倍率，不给精确手扒数；`rank_mcns` 必须在媒介选定实际询价机构后返回这些机构按 `(platform, kwUid)` 去重的覆盖并集。仅当该倍率 `<20` 时，精确补量为 `需求数×20−覆盖数` 并提供一键发起；`20≤倍率<30` 为中风险，可询价但建议补资源；`≥30` 为安全，无需手扒。机构集合变化必须重新计算。正式契约见 `spec/algorithms.json`、`spec/workflow.json` 与 CHG-2026-016。
