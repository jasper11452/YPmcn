...
同一条规范化 query 会生成两份相同模型的 query embedding，分别检索 `content` 和 `commercial`。两路结果按 `platform:kwUid` 去重，以最佳名次合并；当前不是分数加权融合。Qdrant 层只按 `platform` 过滤，其余硬条件在回源 MySQL 后判断。
...
当前 point ID 由 `platform + kwUid + source_snapshot_date` 生成；同一达人新快照会形成新 point。搜索会按 `platform:kwUid` 在单次召回结果中去重，但旧 point 会持续占用存储，并可能让召回候选偏向重复达人。

必须二选一：

1. **推荐：仅保留当前达人。** point ID 改为稳定的 `platform + kwUid`，upsert 覆盖旧版本，并为删除/下线记录同步删除 point；
...
仅为实际在 Qdrant filter 中使用的字段建索引。`kwUid` 目前只用于回源和去重，不是过滤条件；是否建 keyword index 应由后续对账/删除实现决定，不要为全部 payload 字段盲目建索引。
...
- 确认 Cloud 结果均能按 `platform + kwUid` 回源；
