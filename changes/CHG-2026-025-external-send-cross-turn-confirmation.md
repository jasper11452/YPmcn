...
- `npm run test:headless`：通过；90 项 Plugin 测试和 OpenClaw 源码装载冒烟均通过。
- OpenCode 只读端到端核验：使用 `opencode/laguna-s-2.1-free` 实际执行跨轮生命周期测试，通过（1 pass、0 fail）；未改写工作树，未调用 Provider。
- `npm run verify`：通过；Spec、文档、密钥、Plugin、Provider comparator、Skill 和发布包检查均通过。
- `npm run pack:yp`：通过；生成 `packages/releases/ypmcn-media-assistant-3.4.22.tgz`（139347 bytes，SHA-256 `a0d09a0a8cec2dbe99f7236028132b8871fd4f5e47b4ff63a874a7e60eeeb3fa`），打包后密钥扫描通过。
