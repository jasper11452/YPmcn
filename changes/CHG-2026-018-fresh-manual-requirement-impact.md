...
| Workflow | 拓展达人可从任意既有阶段重新进入需求解析，不再依赖搜索、赛马或字段选择完成 | `validate_requirement` 必须紧邻拓展达人，成功返回的新 ID 只能使用一次。 |
| Skill / Prompt | 删除复用旧需求 ID 与字段选择作为拓展达人前置的指令 | 导出仍需字段选择，但它必须发生在本次需求解析之前。 |
