...
| MCP output evidence | `rank_mcns` 目标成功结构增加 `inquiry_id`；达人拓展任务证据回显同一 `inquiry_id` | 均为 Provider 未广告输出；Hook 必须逐字段验证，不能把普通 envelope 当询价或启动证据。 |
| Workflow | 高风险供给增加达人拓展、仅 MCN、调整数量三条分支；拓展分支统一先 rank、后达人拓展 | 供给确认与企微外发确认完全分离。 |
| Hook state | 保存风险、缺口、建议数量、`rank_mcns` 询价证据及脱敏任务投影 | 本地状态不是 Provider 业务事实，不向用户展示询价或任务 ID。 |
| Skill / Prompt | 改为先创建询价关联，再启动或复用达人拓展任务 | 禁止回退到硬缺口 `0`、反向调用或旧的一字段调用。 |
...
- `rank_mcns` 还必须先原子创建/解析当前需求的询价关联并返回 `inquiry_id`；只对齐达人拓展输入 Schema 仍不能上线。
...
- Agent 只发送需求 ID 和用户确认的数量，不把 `inquiry_id` 作为未声明参数发送，也不发送搜索上下文、风险推断、人工结果、操作人或平台字段。
- Hook 事件只保存询价 ID、任务 ID、状态、数量、操作和时间等最小投影，不保存客户 Brief、达人拓展原始数据或候选明细。
