...
approval_basis: "用户明确要求手扒数量决策放在 MCN 赛马之后，并批准 20/30 倍风险阈值及缺口公式"
baseline: "a086eabbf6bc41adff00bb58560f37423f47b2c2"
rollback_strategy: "回退 3.4.3 插件与本变更 Spec；不得删除已创建的远程询价或手扒任务"
```

## Problem

3.4.1 在 `search_creators` 后、`rank_mcns` 前展示并确认精确手扒数量。此时只能看到全资源库刊例覆盖，尚未确定实际询价机构，也无法得到这些机构覆盖达人的去重并集；提前给出精确数量会把资源库倍率误当成已选机构倍率。
...
3. 赛马前 `<20` 强烈建议先扩机构或预手扒到至少 20 倍；中风险允许继续赛马但建议补资源；安全档无需手扒。
...
6. 高风险赛后弹窗提供“一键发起手扒补量”；提交后才把上述精确缺口保存为 `pending_manual_target_count` 并调用 `manual_source_creators`。
...
```yaml
goal: "把精确手扒决策移到实际询价机构覆盖去重并集计算之后，并落实 20/30 倍风险边界"
allowed_paths:
...
forbidden_paths:
  ...
  - "packages/releases/**"
acceptance:
  - "赛马前只展示刊例倍率与风险建议，状态中不存在精确手扒 target_count"
