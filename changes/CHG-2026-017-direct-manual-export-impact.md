...
| --- | --- | --- |
| MCP | 对齐字段选择、拓展达人、排序输入，并定义三参数导出目标 | 只传契约字段；数值型业务量以正整数十进制字符串传输。 |
| Workflow | 增加字段选择优先的四步直接链 | 任一步失败都不得调用下一业务 Tool。 |
| Skill / Prompt | 删除旧的赛马、询价回收和宿主 CSV 导出前置 | 字段、需求、数量和批次号均复用本轮已确认值。 |
| Hook state | 记录字段选择后的下一步、拓展达人返回的询价集合和导出完成态 | 本地状态不伪造 Provider 成功；未知写结果不盲重试。 |
| Database boundary | 拓展达人按需求直接生成询价结果，排序去重后按需求/数量/批次导出 | Provider 的事务、幂等和文件生成不在本仓库。 |
...
- 旧赛马/企微工具仍保留在 MCP 工具目录，但不属于本次拓展达人导出主链。

## Verification

本地契约、Hook 回放、Skill 校验与完整离线门禁均已通过。生产 Provider 只读比较器确认字段选择、拓展达人和排序输入已对齐，但三参数导出仍未部署；生产导出继续阻断，不自动回退旧 `run_id`。
