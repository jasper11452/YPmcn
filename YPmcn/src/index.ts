...
import {
  ...
  recordManualCreatorListDisplay,
...
- Map confirm_mcn_return_completed to one native popup with header “机构回填确认” and choices “确认已完成回填” / “尚未完成，继续等待”. Never infer completion from inquiry_id presence, sync success, elapsed time, or provider wording; only the human confirmation releases rank_creators.
...
- Keep one requirement primary key through search_creators -> rank_mcns -> select_inquiry_form_fields -> create_with_distributions -> sync_mcn_inquiry_status. Field selection never authorizes a new validate_requirement call. rank_mcns.id and every later requirement_id must equal the data.id that was searched for that execution unit; a new or fallback ID is a lineage error.
- Build description and the identical wechat_notification_message as recipient-ready multiline WeChat text from the structured payload and mapped audit atoms accepted by validate_requirement. Treat that validation-time notification snapshot as the only source; never parse or reinterpret originalBrief again at send time. Include every applicable section and exact fact: 项目/产品、招募数量、预算报价、提报截止、内容方向/合作形式、账号画像/各方向人设、数据要求 and other explicit constraints. Never reduce the message to only project, count, budget, and deadline. The final confirmation must display this complete message.
- Never interpret sync_mcn_inquiry_status as WeCom send evidence. A sync response may record inquiry synchronization or returned inquiry IDs only; it cannot prove that any message was sent, even if its payload contains generic success wording or sent-like fields. Report and persist WeCom success only from a successful create_with_distributions response containing explicit per-supplier sent details for the exact request. Without those details, sent count remains unproven and sync must not advance the local workflow.
- Every create_with_distributions attempt, including each single-supplier fallback, requires its own final native “企微外发确认” popup. Persist popup_required, approved, in_flight, and consumed confirmation states in the local workflow across the AskUserQuestion turn boundary; after an exact “确认发送” answer, the latest unexpired receipt authorizes the next create_with_distributions call once without rechecking parameter equality. Do not reopen a popup solely because the assistant turn or parameters changed. An after-tool result without the matching prompted-and-approved receipt is invalid and must never be recorded or reported as a send.
- Once search_creators starts, complete rank_mcns, the one user-operated field selector, confirmed create_with_distributions, and the send-bound sync step before manual sourcing; this MCN branch is optional only before search starts.
...
- Explicit same-platform 账号方向 are separate search execution units. Inherit shared hard conditions into every unit, but map only that unit's persona heading and subtype labels to kolPersonaLabel and its detailed direction text to description. Validate, search, and rank each unit with its own returned data.id; never merge 专业向、高端向、宝宝形象 into one generic search.
- Search-label compatibility is platform-specific. For Xiaohongshu, map 好物推荐/产品测评 and similar content styles to contentFeatureLabel; contentThemeLabel is Douyin-only and must never be sent in a Xiaohongshu payload because the Provider excludes it. Keep persona terms in kolPersonaLabel, Xiaohongshu blogger taxonomy in pgyBloggerTypeLabel, and Douyin taxonomy in xtTalentTypeLabel.
- Map explicit upper-bound performance constraints instead of preserving them: Xiaohongshu 图文 CPE -> cpeL1, 视频 CPE -> cpeL2, and a Brief covering both formats maps both; CPV x converts exactly to the corresponding CPM upper bound x*1000. Map 女性用户/粉丝占比下限 to femaleRate. A composite age interval such as 25-34, 活跃粉丝占比, or 发现页占比 has no exact current hard-filter field; put its verbatim condition into description as a searchable soft constraint and keep it in the audit atom rather than silently preserving or inventing a false hard mapping.
...
- A successful manual_source_creators response must contain a non-empty creator list using its fixed fields. Immediately render every returned row as one Markdown table with columns 平台、达人ID（douyinId/xiaohongshuId by platform）、达人昵称、内容标签、主页链接; use “-” only for missing returned values and never invent rows. Append the exact HTML comment named by local workflow.manual_sourcing_display_marker after the table so the next prompt can audit that display. The local state separately records creator data received and creator list displayed.
- For rank_creators, omit inquiry_ids (or use null) when no prior verified create_with_distributions result returned an inquiry_id. Otherwise pass exactly one element: the most recent returned inquiry_id, scanning older send results only when newer results have no usable ID. If the active flow has a verified WeCom send, sync must run after create_with_distributions; after manual sourcing, invoke the “机构回填确认” popup and only a human “确认已完成回填” permits ranking the combined pool. inquiry_id presence never proves return completion.
...
- Current production hard-blocks create_submission_batch and get_workflow_state. Do not invoke either tool: never derive submission_batche_page or columns from the approved export input, and never derive requirement_id from trace_id or demand_id plus demand_version. Return integration_required until Provider deploys the approved input contracts.
  ...
  return definePluginEntry({
    ...
    api.on("before_prompt_build", async (event, ctx) => withStateScope(hookStateScope(event, ctx), () => {
      ...
      const splitContext = previews.length > 1
        ? splitReady
          ? `YPmcn authoritative same-platform direction/variant execution units. Process every payload in this exact order. For each unit, keep its returned data.id and immediately complete search_creators then rank_mcns for that same ID before moving to another unit; never validate all units and search only the last one, merge persona labels, substitute a new ID, or skip a unit.\n${JSON.stringify({ payloads: splitPayloads })}`
          : `YPmcn authoritative same-platform direction/variant previews. Resolve all units in one AskUserQuestion popup before validation; do not merge persona directions or skip units.\n${JSON.stringify({ previews })}`
        : "";
