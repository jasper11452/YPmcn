...
- requirement_ready + allowed search_creators => search_creators({id}). After success, do not call or probe rank_mcns first. The very next Tool must be native AskUserQuestion with header “供给确认” and the two options “确认供给方案” and “调整方案”. The Hook binds the latest successful search receipt, replaces the question body with the compact authoritative ten-field supply plan, and corrects stale unfiltered counts from total_matched, supply_assessment, and returned creator-to-supplier bindings. Do not reconstruct the long search output or copy its values into the popup; a short placeholder question is enough. If no valid bound plan exists, stop with integration_required. Never call rank_mcns merely to obtain a marker. Only an explicit “确认供给方案” answer permits rank_mcns({id, platform}).
