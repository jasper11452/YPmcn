...
function postValidationIntent(prompt: string, messages: unknown): "manual" | "search" | undefined {
  if (/(?:手扒|人工(?:筛|找|搜)|manual\s+sourc)/iu.test(prompt)) return "manual";
  if (/^\s*(?:继续|好的?|可以|开始|确认)\s*[。.!！]?\s*$/u.test(prompt) && Array.isArray(messages)) {
    ...
    if (previousAssistant && /(?:手扒|人工(?:筛|找|搜)|manual\s+sourc)/iu.test(messageText(previousAssistant))) {
      return "manual";
    }
  }
...
- Scan all shared and platform-specific requirements before pausing. If concrete required values are still absent or semantically ambiguous, call one native AskUserQuestion containing every necessary question; never split dependent clarification across several popups.
- Ask one question per shared missing value, not one copy per platform. For quantity, ask how many creators each platform needs and accept one shared choice or typed platform-specific counts in that same question.
...
- Collect every independently unresolved requirement value in one self-contained popup. For dependent choices, combine them into one question or provide all valid combined choices; do not open a second popup merely because the first answer selects a platform or price family.
...
- Map evidence-bound gates exactly: confirm_pre_race_supply -> header “供给确认” with “开始MCN赛马” or “仍继续MCN赛马” and the applicable pause option; confirm_post_race_manual_sourcing -> header “赛后补量” with “一键发起手扒补量” / “追加机构后重新计算” / “暂不补量，继续询价”; confirm_mcn_selection -> header “MCN确认” with “确认MCN方案”; confirm_inquiry_fields -> header “字段确认” with “确认字段”. Invoke the matching popup immediately from actual evidence; never print these as a prose menu.
- The select_inquiry_form_fields webpage callback is Tool-owned interaction. After it returns, consume its actual result directly and do not add a redundant chat confirmation.
...
- The Hook checks only this one-time fresh-ID binding for manual sourcing. Do not check whether that requirement was searched before, and do not require field selection, search, MCN racing, distribution, or any other workflow step to be complete. A numeric or wrong-namespace ID is corrected from the existing validate response without another validation. If the Hook returns INTEGRATION_REQUIRED because before_tool_call lacks session context, stop and wait for a host upgrade; never revalidate or retry. A well-formed mismatched, intervened, or already consumed ID requires a new requirement validation.
