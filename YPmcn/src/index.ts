...
- Immediately after rank_creators succeeds, call the host export_csv tool with the first ranked list; do not wait for user confirmation and do not call create_submission_batch first. Build the CSV columns from the exact columns array previously passed to create_with_distributions: preserve array order, use each non-empty columns[].name verbatim as its header and the corresponding columns[].key as the row-value source, and never append, remove, rename, or reorder fields. Use only actual MCP-returned facts, keep missing values empty, and name this first file ypmcn_submission_<demandId>_v<demandVersion>_batch_1.csv. After the export, recommendation_ready: audit_manual_adjustment only for explicit adjustments with reason/operator; otherwise create_submission_batch({run_id}) after the user confirms the recommendation. Omit submission options unless explicit. submission_batch_ready + concrete client feedback => record_client_feedback({run_id,feedback_items}); never infer feedback status.
