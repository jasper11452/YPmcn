...
- Requirement clarification must immediately use one self-contained native AskUserQuestion popup. Use a 2–8 character Chinese topic header for each question, set multiSelect=false, and make the body one direct 4–42 character question ending in “？”, such as “本次需要提报多少位达人？”. YP Action collapses line breaks, so do not add background summaries, “已确认/请确认”, an impact paragraph, raw field names, Brief/gate/status/Tool terms, or the full Brief. Use one question per independent decision; when there are 2–3 independent missing values, send 2–3 questions in the same call so the host renders its question wizard. Each question offers 2–4 option objects with a concise label and plain-language description; include every distinct business choice instead of dropping one to meet a smaller option count. YP Action automatically shows the typed “其他” input when the choices do not fit; do not add “其他” as an option. Never ask for database IDs, trace IDs, or idempotency keys. A denied, cancelled, closed, or timed-out popup is not confirmation and must not continue. Continue the selected safe path in the same assistant turn only after a submitted answer; never ask the user to type “继续”.
...
- Every native AskUserQuestion popup is a user-facing decision screen, not a log dump. Requirement questions use the enforced one-sentence form above; marked supply/send confirmations remain within 320 characters and use 3–5 short visible sections. Keep headings, labels, and descriptions concise. Never expose internal hashes or dump IDs, raw fields, values, provenance, and impact into an undifferentiated paragraph.
...
- requirement_ready + allowed search_creators => search_creators({id}). After success, do not call or probe rank_mcns first. Immediately compute the full supply plan from the returned candidate data. The supply plan has ten fields: demand_count (quantityTotal from the requirement); database_candidate_count (distinct hard-filter-passed creators from the search result); supply_demand_ratio=database_candidate_count/demand_count; target_submission_count (your recommended total submission target); estimated_valid_return_count (your expected valid returns); estimated_gap_count=max(0,target_submission_count-estimated_valid_return_count); recommended_mcn_count (how many MCNs to approach); mcn_covered_creator_count (how many candidates have a supplier/MCN binding via the returned supplier_id field, counted from the actual search result); recommended_manual_creator_count=max(ceil(demand_count*20%),estimated_gap_count); mcn_manual_creator_ratio=mcn_covered_creator_count:recommended_manual_creator_count; and 建议手扒占比=recommended_manual_creator_count/(mcn_covered_creator_count+recommended_manual_creator_count)*100%. Institution count and creator count must never be divided. Do not estimate from pagination, candidate array length, or prose such as “约 500+”. If the Provider returns no viable candidate data at all, stop with integration_required. If test-only values are explicitly authorized, inspect the real output first, label every derived or fabricated value synthetic_test_only, and never attribute it to Provider. The very next Tool after search_creators must be native AskUserQuestion with header “供给确认”. Use this EXACT fixed format for the question body — it must contain exactly these three 【分区】 sections with ｜ separators on a single visual line (YP Action collapses whitespace):
...
【影响】确认后写入MCN排序
The 建议手扒占比 is the percentage of manually sourced creators in the total recommended pool, calculated as recommended_manual_creator_count/(mcn_covered_creator_count+recommended_manual_creator_count)*100%. The two options must be exactly “确认供给方案” and “调整方案”. The Hook binds this first popup directly to the successful search result, so never call rank_mcns merely to obtain a confirmation marker. Only an explicit “确认供给方案” answer permits rank_mcns({id, platform}).
  ...
  return definePluginEntry({
      ...
      return {
        ...
        prependContext: [
          ...
          preview && preview.gate !== "ready"
            ? `YPmcn mandatory unresolved-Brief interaction: call native AskUserQuestion now and do not return a plain text-only clarification. Build one user-facing form. For each decision, use a unique 2-8 character Chinese topic header, set multiSelect=false, and write one direct 4-42 character question ending in ？, such as 本次需要提报多少位达人？ YP Action collapses line breaks, so do not add background summaries, 已确认/请确认, raw field names, Brief/gate/status/Tool terms, an impact paragraph, or the full Brief. Use one question per decision and put 2-3 independent questions in the same call so the host renders its question wizard. Give every question 2-4 option objects with a 2-12 character label and a 2-36 character plain-language description, preserving every distinct business choice. The host automatically provides typed “其他” input; do not add it as an option. Do not call validate_requirement until every value is concrete. A denied/cancelled/closed popup does not confirm anything. After a submitted answer, continue in this same interaction without asking for “继续”.\n<YPmcnClarificationAuthority>\n${renderStandardBriefReply(preview)}\n</YPmcnClarificationAuthority>`
            : "",
        ].filter(Boolean).join("\n\n"),
      };
    });
    ...
    api.on("before_tool_call", async (event, ctx) => {
      try {
        ...
        const previousBlock = isAskTool(raw) ? undefined : blockedToolTurnFailure(rootDir);
