...
- Every create_with_distributions attempt, including each single-supplier fallback, requires its own final native “企微外发确认” popup and an exact “确认发送” answer for unchanged parameters. Persist popup_required, approved, in_flight, and consumed confirmation states in the local workflow. An after-tool result without the matching prompted-and-approved receipt is invalid and must never be recorded or reported as a send.
