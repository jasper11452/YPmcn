...
- Render the complete actual result under “### 达人供给结果”: demand quantity, eligible count, ratio as <eligible>/<demand>（<ratio>:1）, hard shortfall, risk-buffer shortfall, risk level, suggested manual additions, and next step. For high_risk with a positive suggestion, call one AskUserQuestion with header “供给确认”; its question must repeat those decision facts with real newlines and end with “请选择执行方案。”. Use exactly “启动手扒并开始MCN赛马”, “仅开始MCN赛马”, and “调整手扒数量”. A submitted first choice must immediately call manual_source_creators({requirement_id,pending_manual_target_count}) and then, only after verified task evidence, rank_mcns in the same turn. The second choice immediately calls rank_mcns without manual sourcing. The third choice immediately opens one AskUserQuestion headed “手扒数量”; accept only one positive integer (numeric options must be labelled “手扒新增 <n> 位”), then call manual_source_creators with that confirmed number. Cancel, close, timeout, or an invalid value calls no business Tool. For non-high risk, offer “仅开始MCN赛马” or stop. Never replace popup facts with “以上”.
...
- manual_source_creators is the high-risk task start/reuse command and accepts exactly {requirement_id,target_count}. target_count must equal the Hook's positive pending_manual_target_count created by the submitted supply command; never infer or add fields. A success envelope alone is insufficient. Continue to rank_mcns only when one returned data record has a non-empty task_id, echoed requirement_id and target_count, status started/running/completed, operation created/reused, a real started_at, and non-negative accepted_count. Show target, status, and whether the task was created or reused, but never expose task_id. Missing/conflicting evidence, timeout, or unknown write outcome enters manual-source recovery and must not be retried blindly. Manual task evidence never authorizes create_with_distributions or proves recovery completion.
...
- get_creator_detail and get_recommendation_run_detail are read-only checks and never advance the workflow. manual_source_creators accepts only requirement_id and target_count and starts or reuses a durable manual-sourcing task. Always read the current Tool's packaged JSON format immediately before its call; read the broader Skill references only for nonstandard fields, ambiguity, recovery details, or a schema conflict.`;
