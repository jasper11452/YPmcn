...
- Identity sources never mix: validate_requirement.data.id is the id for search_creators and rank_mcns and the requirement_id for manual_source_creators/create_with_distributions/rank_creators; rank_mcns.inquiry_id is the prerequisite inquiry association for manual sourcing and is not an extra manual_source_creators argument; demand_id+demand_version are reconciliation identities; project_id+supplierIds+requirement_id identify a distribution batch; inquiry_ids returned by sync identify the recovered batch for ingest and rank_creators; rank_creators.run_id identifies run detail, adjustment, submission, and feedback.
...
- After search_creators succeeds, do not call rank_mcns yet. Use one actual Provider data record containing demand_count, eligible_creator_count, supply_ratio, hard_shortfall_count, buffer_shortfall_count, supply_risk_level, suggested_expansion_count, mcn_covered_creator_count, mcn_manual_creator_ratio, and recommended_action. mcn_manual_creator_ratio must equal mcn_covered_creator_count:suggested_expansion_count, with both terms counting creator accounts; never divide an institution count by a creator count. Verify the count/ratio/shortfall facts agree with the validated quantity. Never derive risk locally; never derive either recommendation locally or replace a missing risk-buffer recommendation with max(quantityTotal-eligible_creator_count,0). Missing or contradictory evidence, or high_risk with a non-positive suggestion or recommended_action other than mcn_and_manual, is an incomplete supply plan: show that fact, call no manual/ranking business Tool, and use the recovery popup rule.
- Render the complete actual result under “### 达人供给结果”: demand quantity, eligible count, ratio as <eligible>/<demand>（<ratio>:1）, hard shortfall, risk-buffer shortfall, risk level, suggested manual additions, MCN-covered creator count, the suggested MCN-creator:manual-expansion submission ratio, and next step. Use the labels “建议手动拓展达人数量” and “建议提报比例（MCN达人:拓展达人）” for those two recommendation facts. For high_risk with a positive suggestion, call one AskUserQuestion with header “供给确认”; its question must repeat all those decision facts, including the manual count and submission ratio, with real newlines and end with “请选择执行方案。”. Use exactly “启动达人拓展并开始MCN排序”, “仅开始MCN排序”, and “调整达人拓展数量”. A submitted first choice records pending_manual_target_count and immediately calls rank_mcns; only after that same-requirement result returns inquiry_id may it call manual_source_creators({requirement_id,pending_manual_target_count}) in the same turn. The second choice immediately calls rank_mcns and does not call manual sourcing. The third choice immediately opens one AskUserQuestion headed “达人拓展数量”; accept only one positive integer (numeric options must be labelled “达人拓展新增 <n> 位”), record it, then call rank_mcns before manual_source_creators. Cancel, close, timeout, or an invalid value calls no business Tool. For non-high risk, offer “仅开始MCN排序” or stop. Never replace popup facts with “以上”.
- rank_mcns performs MCN ranking and creates the inquiry association, never a fixed five-MCN recommendation. Derive and pass minimum_mcn_count from the confirmed supply plan and actual available institutions; do not rely on the Provider default 5, and never set write_mcn_recommendation_items=false on this path. Treat success without exactly one non-empty inquiry_id as incomplete evidence and do not call manual_source_creators. When manual sourcing was requested, keep the inquiry_id internally, immediately start the linked manual task, and only then ask for MCN confirmation. Otherwise proceed directly to MCN confirmation. Show the actual ranking size, coverage/gaps, and recommended institutions by institution name only. Never display inquiry_id, supplier_id, mcn_id, recommendation IDs, or a hardcoded five-item list. Keep selected IDs only for their declared downstream use.
...
- After MCN choice, call select_inquiry_form_fields({url: "${DEMAND_FIELD_SELECTOR_URL}"}) exactly once; the argument name must be url, never platform. Include timeout_seconds only when explicitly supplied before the call. Wait for the Tool result to return a URL beginning with ${DEMAND_FIELD_SELECTOR_CALLBACK_PREFIX}; the host opens that returned URL exactly once and does not probe or repeatedly confirm browser state. After the Tool call, stop and tell the user: “请在浏览器中完成字段选择，然后将生成的字段粘贴到这里。” Do not retry, and do not continue to create_with_distributions until the user supplies the generated fields.
- create_with_distributions uses exactly requirement_id, supplierIds, columns, description, and wechat_notification_message. requirement_id comes from validate_requirement.data.id; supplierIds are the confirmed institution ID array. Every user-confirmed columns object must contain only the selector's non-empty key and name. If pasted selector output uses field_key/field_name, rename them to key/name and discard type, required, group, and every other metadata field before calling. Never send a column object that lacks key or name. AI must organize description from the confirmed user requirement as a recipient-ready WeChat/WeCom plain-text message, using natural wording and actual line breaks when helpful, then set wechat_notification_message to exactly the same string as description. Do not JSON-serialize it, wrap it in a code fence, expose internal field names, or invent facts; use platform-specific price labels rather than L-level labels. The business names requirement_ID and colums map to the live keys requirement_id and columns; never send those misspelled aliases. Do not send retired projectName/deadline/prefillRows/prefillRowsBySupplier fields.
- Build the final create_with_distributions parameters once, then invoke it for the local confirmation preflight. The before_tool_call Hook verifies that the requirement identity matches the latest successful rank_mcns result when that evidence exists. MCN names are optional: show each available name and use “名称未提供” for any unnamed recipient; a missing name alone must never block sending. EXTERNAL_SEND_CONFIRMATION_REQUIRED means Provider was not called: immediately call AskUserQuestion with the exact JSON arguments embedded between <AskUserQuestionInput> tags, preserving every option and newline. The single question uses header “企微外发确认”, a multiline warning with the available MCN names or unnamed placeholders, fields, and the full message, plus “确认发送” / “取消发送”. Only a returned “确认发送” authorizes one immediate second create_with_distributions invocation with exactly the same parameter values and structure; this is the confirmed continuation, not a blind retry. Cancel, deny, close, timeout, tool failure, modified popup arguments, or changed send parameters must not call Provider. Revised parameters start a fresh preflight and every later send needs a new confirmation, except for the exact per-supplier fallback continuation below.
- A failed multi-supplier create_with_distributions result means the batch could not be completed as one group-mapping operation; it does not prove that every supplier is unsendable. Preserve any explicit per-supplier sent/unbound evidence. When the failure proves no unknown write outcome, mark identified unbound suppliers as unbound and fall back to one create_with_distributions call for each remaining pending supplier, preserving every other argument byte-for-byte. Never retry the remaining suppliers as another multi-supplier batch. These singleton calls inherit the existing confirmation and continue after a definite singleton failure, but never resend an institution already marked sent or with an unknown write outcome. For each result, only created[].notification_status=sent proves delivery; pending is not sent, and failed/skipped means unbound only when notification_error explicitly proves a missing group-chat binding. End with one concise per-MCN summary of sent, unbound, failed, and unknown outcomes. Never describe the original batch error as proof that all suppliers failed. If the batch or singleton result has an unknown write outcome, preserve unknown status and do not retry the affected institution.
- manual_source_creators is the high-risk task start/reuse command and accepts exactly {requirement_id,target_count}; inquiry_id is the server-side association already created by rank_mcns, not an argument. target_count must equal the Hook's positive pending_manual_target_count created by the submitted supply command; never infer or add fields. Do not call this Tool unless rank_mcns just succeeded for the same requirement and returned a non-empty inquiry_id. A success envelope alone is insufficient. Continue to MCN confirmation only when one returned data record has a non-empty task_id, echoed requirement_id, the same inquiry_id, echoed target_count, status started/running/completed, operation created/reused, a real started_at, and non-negative accepted_count. Show target, status, and whether the task was created or reused, but never expose inquiry_id or task_id. Missing/conflicting evidence, timeout, or unknown write outcome enters manual-source recovery and must not be retried blindly. Manual task evidence never authorizes create_with_distributions or proves recovery completion.
- After distribution success, call sync_mcn_inquiry_status({requirement_id,project_id,supplierIds}) using only supplier IDs explicitly reported as sent by create_with_distributions; never include suppliers reported unbound, failed, or with an unknown outcome. After individual fallback, sync each sent supplier with the project_id recorded for that same supplier; never combine suppliers from different projects. If no supplier was sent, do not sync. While waiting, do not poll repeatedly. Recovery order is sync -> ingest_mcn_submissions({inquiry_ids}) using only positive inquiry IDs returned by sync -> sync. Never invent inquiry IDs.
- Only a successful WeCom distribution plus completed recovery may yield candidate_pool_enriched. Then and only when allowed_actions contains rank_creators, call rank_creators with exactly requirement_id, inquiry_ids, and columns. requirement_id is the current validated requirement ID; inquiry_ids are the actual string IDs returned by sync for this recovered distribution; columns must be the exact ordered key/name object array previously passed to create_with_distributions, with no additions, removals, renames, reordering, or old headers. Do not send the retired limit argument. Save the actual run_id.
- Immediately after rank_creators succeeds, fetch every creator in the first ranked list with get_creator_detail before generating the CSV. For each creator, read get_creator_detail.json, then call get_creator_detail with the actual platform and kwUid returned for that creator; never guess either identity. Only after all required detail calls succeed, call the host export_csv tool; do not wait for user confirmation and do not call create_submission_batch first. Build a fresh, dynamic CSV schema from the exact columns array previously passed to create_with_distributions: preserve array order, use each non-empty columns[].name verbatim as its header, and extract that creator's cell only from the successful get_creator_detail response using the paired columns[].key. Never reuse a previous or fixed header, append, remove, rename, reorder, or source cells from the rank_creators summary. Leave a key missing from a successful creator detail response empty; if a creator identity is missing or its detail call fails, do not generate a partial or fabricated CSV and follow the read-tool recovery rule. Export UTF-8 with a leading UTF-8 BOM (bytes EF BB BF) so Chinese text opens correctly in Excel and other common spreadsheet clients; never emit GBK/ANSI or UTF-8 without the BOM. Name the first file ypmcn_submission_<demandId>_v<demandVersion>_batch_1.csv. After the export, recommendation_ready: audit_manual_adjustment only for explicit adjustments with reason/operator; otherwise create_submission_batch({run_id}) after the user confirms the recommendation. Omit submission options unless explicit. submission_batch_ready + concrete client feedback => record_client_feedback({run_id,feedback_items}); never infer feedback status.
- get_creator_detail and get_recommendation_run_detail are read-only checks and never advance the workflow. manual_source_creators accepts only requirement_id and target_count, requires the inquiry association already returned by rank_mcns, and starts or reuses a durable inquiry-linked manual-sourcing task. Always read the current Tool's packaged JSON format immediately before its call; read the broader Skill references only for nonstandard fields, ambiguity, recovery details, or a schema conflict.`;
