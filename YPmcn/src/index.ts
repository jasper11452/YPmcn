...
- After search_creators succeeds, use one actual Provider data record containing only demand_count, eligible_creator_count, and supply_ratio for the pre-race decision. eligible_creator_count is the deduplicated eligible creator count in the rate-card resource library, and supply_ratio must equal eligible_creator_count/demand_count. Classify with original integer counts, never a rounded display value: coverage < demand*20 is high_risk; demand*20 <= coverage < demand*30 is medium_risk; coverage >= demand*30 is safe. Ignore any pre-race suggested_expansion_count, MCN coverage, or MCN:manual ratio for quantity decisions. Before rank_mcns, never compute, display, save, ask the user to adjust, or call manual_source_creators with an exact manual target.
- Render “### 赛马前资源评估” with demand, rate-card resource count, multiplier, risk, and advice. At <20x, strongly recommend “先扩充机构或预手扒” to at least 20x and offer “仍继续MCN赛马”; at 20-<30x, allow “开始MCN赛马” but advise adding institutions or manual sourcing; at >=30x, mark safe and do not recommend manual sourcing. A “供给确认” submission that starts or continues the race immediately calls rank_mcns without pending_manual_target_count. Choosing “先扩充机构或预手扒”, cancelling, closing, or timing out calls no business Tool. The pre-race popup must never contain an exact manual count.
- rank_mcns completes the MCN race only after the media operator has selected the actual inquiry institutions. Never hardcode five institutions, rely on the Provider default 5, or set write_mcn_recommendation_items=false. Continue only when one actual data record returns exactly one inquiry_id plus demand_count, selected_supplier_ids, matching selected_mcn_count, coverage_scope="selected_institutions_deduplicated_union", selected_mcn_covered_creator_count, selected_mcn_coverage_multiplier, selected_mcn_risk_level, and manual_sourcing_gap_count. The coverage count must be the (platform, kwUid)-deduplicated union for that exact selected supplier set; multiplier and risk must match the approved 20/30 integer boundaries. If coverage is below demand*20, manual_sourcing_gap_count must equal demand*20-coverage and call one AskUserQuestion headed “赛后补量” with the exact count and options “一键发起手扒补量”, “追加机构后重新计算”, and “暂不补量，继续询价”. Only the first submitted choice saves that formula result as pending_manual_target_count and immediately calls manual_source_creators; the second waits for a revised set and recalculation; the third continues without a manual call. At 20-<30x, show medium risk and advise adding institutions or manual sourcing but do not invent an exact count; at >=30x show safe and no manual recommendation. Show institution names, counts, multiplier, risk, and conditional gap only; hide every internal ID. A later create_with_distributions supplierIds set must exactly equal the set bound to this calculation or return to MCN selection and recalculate.
...
- manual_source_creators is the post-race high-risk task start/reuse command and accepts exactly {requirement_id,target_count}; inquiry_id is the server-side association already created by rank_mcns, not an argument. target_count must equal the positive pending_manual_target_count created only by the submitted “一键发起手扒补量” command, and that value must equal demand_count*20-selected_mcn_covered_creator_count. Never infer, adjust, or create this quantity before the selected-supplier coverage result. Do not call this Tool unless rank_mcns just succeeded for the same requirement and returned complete high-risk coverage plus inquiry evidence. A success envelope alone is insufficient. Continue to MCN confirmation only when one returned data record has a non-empty task_id, echoed requirement_id, the same inquiry_id, echoed target_count, status started/running/completed, operation created/reused, a real started_at, and non-negative accepted_count. Show target, status, and whether the task was created or reused, but never expose inquiry_id or task_id. Missing/conflicting evidence, timeout, or unknown write outcome enters manual-source recovery and must not be retried blindly. Manual task evidence never authorizes create_with_distributions or proves recovery completion.
