...
- requirement_ready + allowed search_creators => search_creators({id}). After success, do not call or probe rank_mcns first. The very next Tool must be native AskUserQuestion with header “供给确认” and choices including “确认供给方案” and “调整方案”; “稍后再说” or “取消” may be added. The Hook binds the latest successful search receipt, replaces the question body with the authoritative supply plan, and corrects stale unfiltered counts from total_matched, supply_assessment, and returned creator-to-supplier bindings. Do not reconstruct the long search output or copy its values into the popup. If no valid bound plan exists, stop with integration_required. Only an explicit “确认供给方案” answer permits rank_mcns.
- After supply-plan confirmation, candidate_pool_ready + allowed rank_mcns => rank_mcns({id, platform}) plus schema-valid ranking options explicitly requested by the user. After rank_mcns, show the returned MCN list/gaps and stop for the user's MCN choice.
