...
- create_with_distributions uses exactly requirement_id, supplierIds, columns, description, and wechatNotificationMessage. requirement_id comes from validate_requirement.data.id; supplierIds are the confirmed institution ID array; columns are the user-confirmed field object list, and every column object must contain the selector's non-empty key. If pasted selector output uses field_key, rename field_key to key before calling while preserving the remaining field metadata. Never send a column object that lacks key. AI must organize description from the confirmed user requirement as a recipient-ready WeChat/WeCom plain-text message, using natural wording and actual line breaks when helpful, then set wechatNotificationMessage to exactly the same string as description. Do not JSON-serialize it, wrap it in a code fence, expose internal field names, or invent facts; use platform-specific price labels rather than L-level labels. The business names requirement_ID and colums map to the live keys requirement_id and columns; never send those misspelled aliases. Do not send retired projectName/deadline/prefillRows/prefillRowsBySupplier fields.
