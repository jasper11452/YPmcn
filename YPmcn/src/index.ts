...
- If no verified WeCom send exists in this flow, continue directly with rank_creators using inquiry_ids=[] and then create_submission_batch. If verified WeCom send exists, sync must run after create_with_distributions; after manual sourcing, invoke the “机构回填确认” popup and only a human “确认已完成回填” permits ranking the combined pool and creating the submission batch. inquiry_id presence never proves return completion.
