...
function searchSummary(root: Json): Json {
  ...
  const prices = ["price_cents", "kolOfficialPriceL1", "kolOfficialPriceL2", "kolOfficialPriceL3", "downloadPriceL1", "downloadPriceL2", "downloadPriceL3"];
  const rebates = ["default_rebate_rate", "rebate_min_rate", "rebate_max_rate"];
  return {
    candidateCount: creators.length,
    withCreatorPriceCount: creators.filter((item) =>
      prices.some((field) => item[field] != null) ||
      (Array.isArray(item.creator_prices) && item.creator_prices.length > 0)
    ).length,
    withSupplierRebateCount: creators.filter((item) => rebates.some((field) => item[field] != null)).length,
  };
}
...
export function afterTool(event: Json, ctx: Json, rootDir: string): void {
  ...
  else if (tool === "rank_mcns" && session.phase === "search_completed" && input.id === session.ids.requirement_id) { const id = explicit(result, "mcn_run_id", "mcn_recommendation_id", "id"); if (!id) issue(session, tool); else { session.phase = "mcn_planning"; session.ids.mcn_run_id = id; } }
