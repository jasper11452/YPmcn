...
- Stop matrix: `next_action=null`, Provider wait, terminal failure without safe recovery, and cancelled Ask may stop. All other nonterminal states continue; human decisions require Ask before stopping. Nonterminal output is Tool calls only.
...
- If the host drops session context after approval, continue only from one exact unexpired local receipt; zero or multiple matches remain blocked.
