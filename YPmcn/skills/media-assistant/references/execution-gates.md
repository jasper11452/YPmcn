...
2. 仅导出时先调用 `select_inquiry_form_fields({platform})`，等待网页提交，并把每行“字段名：备注”转为保持原序的 `{key,name}`；只手扒时不以字段选择为前置。
...
5. 导出时只接受该手扒成功响应中的非空、唯一字符串 `inquiry_ids`，无实际 `inquiry_ids` 不 rank；用它们、相同 `requirement_id` 和本轮 `columns` 调用 `rank_creators`，成功后同轮调用 `create_submission_batch({requirement_id,size,number})`。
...
- 导出场景的字段页面取消、超时或返回无效字段时停止该导出链；不得拿旧字段继续。
