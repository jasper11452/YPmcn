...
MCN 排序方案 `data.id`、MCN 排序列表、当前达人供需关系、建议手扒比例、`inquiry_advice`、建议询价 MCN 列表、累计供给倍数和是否可发送询价。

## 调用后必须停在哪里

先向前端展示当前达人供需关系和建议手扒比例；再停在 MCN/野生比例确认；再停在 MCN 机构名单确认；然后停在表单字段确认。用户同意或修改 MCN 列表后，Agent 根据需求表主键读取非空字段拟写企微消息并请求确认。确认前不得发送企微询价。

## 禁止

不得把 MCN 排序结果当作已发送。不得跳过供需关系展示、手扒比例提示、机构剔除/补充、表单字段确认和企微消息确认。不得为了凑满 5 家放宽硬筛条件或扩充不合格 MCN。不得将中风险默认视为已确认。
