...
- 返点单位**小数**：20% → `rebate_min_rate: 0.2` 且 `rebate_max_rate: 0.2`
- 原文不存在的值传 `null`，不编造

当前继续前必填项：`platform`、`submission_deadline_at`、`raw_messages_json`、`budget_min_cents`、`budget_max_cents`、`budget_raw`、`rebate_min_rate`、`rebate_max_rate`、`rebate_raw`、`quantity_total`。缺任一项时不可继续候选搜索。
...
`status=draft` 时停在补充缺失必填字段。`status=ready` 时先确认额外需求字段映射，再停在结构化 brief 确认，展示平台、数量、deadline、预算/返点区间、内容要求、数据指标、表单字段影响，用户确认后才进入 `search_creators`。

## 禁止

不得为满足 ready 编造平台、数量、deadline、预算、内容要求或返点。不得在额外需求字段未匹配并确认时继续候选搜索。
