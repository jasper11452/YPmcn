...
- 解析字段（`platforms`、`quantity_total`、`submission_deadline_at`、`budget_max_cents`、`content_requirements` 等 `customer_demands` 字段）
- 可选 `raw_messages`（保留用户原文）
- 可选 `project_context`、`existing_demand_id`、`existing_demand_version`

解析字段由 Agent 对照 `references/creator_candidate_pool_schema.csv` 解析用户 Brief 后构造。无独立字段的筛选条件落入 `requirements_json`。
