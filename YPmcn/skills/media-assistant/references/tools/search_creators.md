...
- 需求表只有客户原始 `rebate` 字符串；若后端能按已确认规则解析，可用于筛选，但绝不是机构实际返点。

## 调用后必须停在哪里

展示实际候选摘要，并分别标注“库内达人价格”“供应商默认返点”“缺失值”。随后展示返回的 `supply_plan`：`demand_count`、`database_candidate_count`、`supply_demand_ratio`、`target_submission_count`、`estimated_valid_return_count`、`estimated_gap_count`、`recommended_mcn_count`、`mcn_covered_creator_count`、`recommended_manual_creator_count`、`mcn_manual_creator_ratio`。手扒数按 `max(ceil(demand_count * 20%), estimated_gap_count)`；比例按机构覆盖达人账号数与手扒达人账号数计算。缺少任一非平凡输入时返回 `integration_required`，不得由 Agent 猜测。用户未确认前禁止调用 `rank_mcns`。

## 能力边界

该工具必须把 `database_candidate_count` 定义为按 `(platform, kwUid)` 去重后的硬筛通过账号数，并由服务端生成可追溯的 `supply_plan`；当前开发 Provider 若尚未返回这些字段，供给确认链保持 `integration_required`。达人身份、资料和分档价格从平台达人表读取；达人—机构关系经 `creator_supply_offers` 关联，返点从 `core_supplier.default_rebate_rate` 读取。返回中的 `price_cents` 是兼容字段；单个 `price_cents=null` 不能证明达人表没有价格。向量召回只能作为内部软特征，不能覆盖硬条件。
