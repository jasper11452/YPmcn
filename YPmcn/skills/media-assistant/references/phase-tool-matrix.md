...
2. 按 `reference_schema.csv` 提取平台、预算、返点、DDL、数量及高频筛选字段；不确定值保留原文，不猜测。
3. 调用 `validate_requirement(payload)` 写入/校验需求。
4. 只有返回可证明当前需求标识和 ready 状态时进入搜索；否则停在 `requirement_draft`。
...
`get_recommendation_run_detail`、`get_creator_detail`、`get_workflow_state` 仅查询，不推进 phase。`audit_manual_adjustment` 是仅允许在 `recommendation_ready` 调用且不推进 phase 的业务写。`manual_source_creators` 用于真实人工补量。
