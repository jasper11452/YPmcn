...
2. 把 Brief 拆成原子需求，按 `reference_schema.csv` 的 `Field` 匹配字段、按 `Type` 规范化并校验值、按 `Comment` 判断平台语义；提取平台、达人官方报价档位、项目总预算、返点、DDL、数量及高频筛选字段，不确定或格式不合法的值保留原文或列入歧义，不猜测。
3. 向用户展示完整字段预览、原文保留、歧义和解析评分；只有 `score > 80` 且无硬阻断项时继续。
4. 调用 `validate_requirement(payload)` 写入/校验需求。
5. 只有返回可证明当前需求标识和 ready 状态时进入搜索。返回 Draft、非 ready、缺失、冲突或待确认项时，立即向用户提出最小澄清问题；用户补充后沿用相同 `demandId`、递增 `demandVersion` 再调用，不得另建需求、无反馈等待或使用相同版本空转。
...
`get_recommendation_run_detail`、`get_creator_detail`、`get_workflow_state` 仅查询，不推进 phase。当前没有公开向量工具。`audit_manual_adjustment` 是仅允许在 `recommendation_ready` 调用且不推进 phase 的业务写，但当前没有独立不可变审计流水。`manual_source_creators` 用于真实人工补量。
