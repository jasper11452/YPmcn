...
| `rebate_min_rate` / `rebate_max_rate` | 单位 **小数**。20% → `0.2`，单值写闭区间，**禁止**传 `20` |
| 无独立字段的筛选条件 | 放入 `requirements_json` |

继续前必填项：`platform`、`submission_deadline_at`、`raw_messages_json`、`budget_min_cents`、`budget_max_cents`、`budget_raw`、`rebate_min_rate`、`rebate_max_rate`、`rebate_raw`、`quantity_total`。缺任一项不可继续。

字段名与 CSV `字段` 列完全一致（蛇形命名）。额外需求必须参考 `creator_candidate_pool_schema.csv` 匹配到需求表字段或达人筛选字段，并让用户确认。字段不确定时先问，不落最终筛选条件。

## 调用后

- `status=draft`：展示最多 3 个缺失项/模糊点用弹窗问用户
- `status=ready`：先确认额外需求字段映射，再展示摘要（平台、数量、deadline、预算/返点区间等），弹窗确认后才 `search_creators`
