...
- `rank_mcns` 成功后前端展示达人供需关系、建议手扒比例和建议询价 MCN 列表；用户同意或修改后，Agent 根据需求表主键读取非空字段拟写企微消息
- 发送前停 5 步确认（比例→名单→表单→权限→内容）→ `create_with_distributions`
- `create_with_distributions` 使用固定接口字段；每个 MCN 必须有唯一填报链接，并用 `prefillRowsBySupplier` / `prefill_rows_by_supplier` 预填候选池中属于该 MCN 的达人
- 需要手扒时同步启动 `manual_source_creators`；`create_with_distributions` 成功后停，等机构回填和手扒结果回收到候选池 → 确认对候选池进行达人精排 → `rank_creators`
