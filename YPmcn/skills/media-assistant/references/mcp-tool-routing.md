...
当前没有 `get_workflow_state`。`create_with_distributions` 取代旧 create_mcn_inquiries，是当前唯一企微分发工具名；通过插件端点发送真实企微询价。先 `preview_only: true` 获取预览，确认后 `preview_only: false` 真实发送。项目模板匹配优先使用 `usageScope: "project"`，接口文档里的 `项目` 会被 hook 兼容归一，不要选择其他枚举。不得再调用旧工具或近似内部服务。
