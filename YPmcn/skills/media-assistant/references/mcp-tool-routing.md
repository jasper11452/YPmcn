...
- `rank_mcns` 成功后，MCP 返回达人供需关系（需求数量/候选数量/MCN 覆盖数量/缺口）、建议达人拓展比例及原因、建议询价 MCN 列表；前端必须展示这些信息，让用户同意或修改 MCN 列表
...
- 需要达人拓展时，在 MCN 列表确认后同步启动 `manual_source_creators`（不等 MCN 回填结束）
...
- `create_with_distributions` 成功后停，等机构回填和达人拓展结果回收到候选池 → 确认对候选池进行达人精排 → `rank_creators`
