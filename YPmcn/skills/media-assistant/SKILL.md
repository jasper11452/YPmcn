...
2. **Brief 入口例外**：直接调 `validate_requirement`，不先弹窗
...
4. 精度（出过错的点）：
   ...
   - 返点 **小数**：20% → `0.2`
   - 原文未提 → `null`，不编造
5. ID、`run_id` 只来自 MCP 成功响应。`demand_id` ≠ `run_id`
6. Schema 缺失或冲突 → 停
...
→ 弹窗 confirm-structured-brief
→ search_creators（之后不得再弹窗确认口径）
→ 弹窗 confirm-supply-ratio
→ rank_mcns
→ 弹窗 mcn-select-for-wechat
→ 弹窗 confirm-form-fields → 弹窗 confirm-wecom-permission
→ 弹窗 mcn-wechat-send（预览消息）
→ create_with_distributions（先 preview_only=true 预览，确认后正式发）
→ 弹窗 proceed-to-ranking
→ rank_creators
→ 弹窗 confirm-risky-submission（有风险账号时）
→ create_submission_batch（复用 rank_creators 的 run_id）
```
...
- 传 ISO 8601 `deadline`，`usageScope: "project"`
- 发送后停，等精排确认

## 响应校验

1. `success` 布尔，`trace_id` 非空
2. `success=true` → `error=null`；反之 `data=null`，`error` 对象
3. `workflow_state` 未返回不判错
