...
- MCP 返回 `status=draft` 时只展示澄清项；返回 `status=ready` 后停在结构化 brief 确认，用户确认后才进入 `search_creators`。

## 调用顺序与安全边界

正常主链路（每个 → 之后必须停下确认，不得自动连续调用）：
...
→ **停：MCN/野生比例确认**
→ **停：MCN 机构名单确认**
→ **停：表单字段确认**（参照 [表单字段映射](references/form-field-mapping.md)）
→ **停：企微角色权限 gate**
→ **停：发送对象/内容/表单/群确认**
...
- 项目模板匹配固定传 `usageScope: "project"`；这是唯一固定值，不使用 `项目`、`campaign`、`supplier` 等其他枚举。漏传时 hook 会补 `project`，显式传错会阻断。
- 其他项目字段按运行时 schema 组织：可用顶层项目字段，也可用 `project` 嵌套对象；`supplierIds`、`sendWechatNotification` 等分发字段放在 schema 要求的位置，不额外虚构 `endpointUrl`、`execute` 或发送模式字段。
