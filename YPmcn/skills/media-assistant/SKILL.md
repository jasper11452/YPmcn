...
- 当前请求 schema 不包含 `trace_id`、`idempotency_key`、`gate_id` 或 `confirmation_type`。`trace_id` 仅由 MCP 在响应中返回。
...
7. 不得向用户索取或自行添加 `trace_id`、`idempotency_key`。
...
- **收到媒介输入后，Agent 必须先对照 `references/creator_candidate_pool_schema.csv` 的"合并结果"列解析用户 Brief 字段**：先满足必填字段，再匹配非必填字段到 CSV 表头，语义模糊的表头和字段值用 `askuserquestion` 弹窗让用户澄清。具体规则见 [需求入口](references/requirement-intake.md)。
- 解析完成后将结构化字段作为 `validate_requirement` 的顶层入参直接传入，不嵌入 `raw_messages`。
- MCP 的 `status=ready` 必须以平台、数量、截止提交时间、内容或单价至少一个为最低业务完整性；返点缺失不阻断。
- `raw_messages` 保留用户原文（可选）。消息对象至少使用 `role` 与 `content`；角色优先使用 `client`/`media`/`agent`/`system`。
