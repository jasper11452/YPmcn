...
3. 通过 `AskUserQuestion` 向用户一次性列出并确认：目标工具、当前 schema 的必填字段、拟传值、缺失或歧义项（参照 [AskUserQuestion 模式](references/ask-user-question-patterns.md) `pre-validate-requirement`）。用户确认前不得调用任何业务工具。
...
| 任何需要暂停等待用户决策的节点 | [AskUserQuestion 模式](references/ask-user-question-patterns.md) — 所有用户交互的单一可信源 |
...
`validate_requirement → search_creators → rank_mcns → 停止确认企微发送 → create_with_distributions → 停止确认精排 → rank_creators → create_submission_batch`
...
- `rank_mcns` 后必须停，通过 `AskUserQuestion` 展示候选比例、选取的 MCN 机构、拟写企微消息，并询问是否发送（参照 `mcn-wechat-send` 模式）。
- 用户确认发送后才调用 `create_with_distributions`；未发送成功前严禁调用 `rank_creators`。
- `create_with_distributions` 成功后再次停，通过 `AskUserQuestion` 询问是否调用 `rank_creators` 精排（参照 `proceed-to-ranking` 模式）；用户确认后才进入 hook 层 `requireApproval` 单次审批。
- `create_submission_batch` 必须复用 `rank_creators` 返回的 `run_id`。
- 任何写调用超时或断连后，不得盲目重试；当前 schema 没有幂等键。优先用详情查询或 `trace_id` 让后端核对。

## 风险确认

- 当业务结果要求确认中风险时，先通过 `AskUserQuestion` 向用户说明风险并等待明确同意（参照 `confirm-medium-risk` 模式）；随后调用 `rank_mcns`，设置 `medium_risk_confirmed: true`。
- 当提报包含 `need_confirm` 账号时，先通过 `AskUserQuestion` 向用户说明风险并等待明确同意（参照 `confirm-risky-submission` 模式）；随后调用 `create_submission_batch`，设置 `allow_need_confirm_with_risk: true`。
...
- 创建项目并分发供应商只走 YP Action 工具 `create_with_distributions`；不得通过 Bash、PowerShell、脚本或 `/api/projects/create-with-distributions/` 直连绕过审批。
- 工具调用必须提供未来的带时区 ISO 8601 `deadline` / `remindAt`。
- 用户确认前不得创建分发或发送通知；审批只接受本次 `allow-once`，拒绝或超时均不执行。 [OpenClaw requireApproval]
